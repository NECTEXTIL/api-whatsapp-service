...
class AuthRepository
{
    ...
    public static function selectUser($userId)
    {
        ...
        if (isset($user['logo_empresa'])) {
            $user['logo_empresa'] = json_decode($user['logo_empresa']);
            if (isset($user['logo_empresa']->{'url'})) {
            }
            $user['logo_empresa'] =
                isset($user['logo_empresa']->{'url'}) ?
                $user['logo_empresa']->{'url'} : null;
        }
        unset($user['password']);
        return $user;
    }
