...
namespace Mnt\mantenedores\Usuario\Domain\Entities;
...
class UsuarioEntities
{
    ...
    {
        $this->service->validateParam('start', 'require start')->isInt();
        $this->service->validateParam('length', 'require length')->isInt();
        $this->service->validateParam('search');
        $this->service->validateParam('order', 'require asc|desc')->isOrder();
    }
